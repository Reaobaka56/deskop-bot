...
<p align="center">
  ...
  Speak a question → Gemini thinks → Answer appears on your LCD.
</p>
...
- 🎙️ **Voice Input** — Hands-free interaction via any PC microphone  
- 🧠 **Gemini 2.5 Flash** — Lightning-fast AI responses from Google's latest model  
- 📟 **LCD Output** — Answers displayed on a physical 16×2 character display  
...
| **2** | Google Speech Recognition converts audio → text |
...
2. Install the **LiquidCrystal_I2C** library → `Sketch → Include Library → Manage Libraries`
...
<table>
  ...
  <tr>
    <td>🔌 USB Cable</td>
    <td>Type-A → Type-B</td>
    <td>For Arduino ↔ PC communication</td>
  </tr>
  <tr>
    <td>🔗 Jumper Wires</td>
...
│
├── 📁 arduino/
│   └── script.cpp            # Arduino firmware (serial → LCD)
│
├── 📁 python/
...
│
├── ⚙️  .env                   # Local secrets (git-ignored)
...
```cpp
LiquidCrystal_I2C lcd(0x3F, 16, 2);  // ← try this
```
</details>
...
| **Display** | 16×2 I2C LCD | Show AI responses physically |
| **Serial Comm** | [PySerial](https://pypi.org/project/pyserial/) | Python ↔ Arduino bridge |
...
<p align="center">
  <strong>Built with ❤️ and an Arduino</strong><br/>
